...
void kirimLokasi() {
 if (gps.location.isValid()) {
    ...
    String message = "ð\xF0\xF0\x9F\xF0\x9F\x93\xF0\x9F\x93\x8D Lokasi Ternak: https://www.google.com/maps?q=" + latitude + "," + longitude;
    sendMessage(message);
  } else {
    sendMessage("â\xE2\xE2\x9A \xE2\x9A\xA0ï\xE2\x9A\xA0\xEF¸\xE2\x9A\xA0\xEF\xB8\xE2\x9A\xA0\xEF\xB8\x8F Gagal mendapatkan lokasi GPS.");
  }
}
...
void cekPermintaanUser() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpCode > 0) {
      String payload = http.getString();
      payload.toLowerCase();
      if (payload.indexOf("kirimlokasi") >= 0) {
        kirimLokasi();
      } else if (payload.indexOf("cekkondisi") >= 0) {
        cekGerakan();
      }
    } else {
...
void loop() {
  ...
  if (threatDetected) {
    sendMessage("â\xE2\xE2\x9A \xE2\x9A\xA0ï\xE2\x9A\xA0\xEF¸\xE2\x9A\xA0\xEF\xB8\xE2\x9A\xA0\xEF\xB8\x8F Deteksi ancaman gerakan tidak biasa pada ternak!");
    kirimLokasi();
    threatDetected = false;
  }
